---
name: fp-plan
description: 财务方案设计与执行——模型推荐、方案生成、协商调整、执行督促。用于为用户设计财务配置方案、协商调整方案参数、跟踪执行进度，当用户需要制定或调整理财方案时使用。
---
...
4. **方案展示话术**
   ...
   **四账户模型展示：**
   ...
   > "- 保本增值 ¥X/月，债券基金/大额存单"
   > "- 高收益 ¥X/月，采用「极简投资」5 ETF 等权配置（沪深300+中证500+标普500+纳斯达克100+债券）"
   > "  具体 ETF 代码和每周定投金额见方案详情。这个方法源自 jane7.com，简单透明，适合投资新手。"
   > "  如果你有偏好的投资策略，可以随时调整——这只是默认推荐。"
   ...
   **核心-卫星模型展示：**
   ...
   > "- 卫星仓（X%）：XX主题，追求超额收益"
   > "每季度检查一次，偏离超 5% 时再平衡。核心仓权益部分参考极简投资等权思路，具体 ETF 代码见方案配置详情。"

   **目标导向模型展示：**
   > "根据你的目标「XX」（时间线 X 年），我用目标导向模型来规划——"
   > "「XX」子账户：每月存入 ¥X（每周定投极简投资 5 ETF 组合），采用 XX 策略"
